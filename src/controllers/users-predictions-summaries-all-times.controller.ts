...
export class UsersPredictionsSummariesAtsController {
  ...
  @response(200, {
    ...
    content: {
      'application/json': {
        schema: {
          type: 'array',
          items: getModelSchemaRef(UsersPredictionsSummariesAts, {
  ...
  async find(
    @param.filter(UsersPredictionsSummariesAts)
    filter?: Filter<UsersPredictionsSummariesAts>,
  ...
  @response(200, {
    ...
    content: {
      'application/json': {
        schema: getModelSchemaRef(UsersPredictionsSummariesAts, {
          includeRelations: true,
        }),
      },
    },
  ...
  async findById(
    ...
    @param.filter(UsersPredictionsSummariesAts, {exclude: 'where'})
    filter?: FilterExcludingWhere<UsersPredictionsSummariesAts>,
  ): Promise<UsersPredictionsSummariesAts> {
  ...
  async getPosition(
    ...
    var sql =
      'SELECT row_position,longestWinStreak,statusWinStreak FROM( SELECT userId,longestWinStreak,statusWinStreak, ROW_NUMBER() OVER(ORDER BY longestWinStreak DESC, percentageWin DESC) AS row_position FROM users_predictions_all_times) AS subquery WHERE userId = ? ';
    var date = new Date();
    var month = date.getMonth() + 1;
    var year = date.getFullYear();

    const results =
      await this.usersPredictionsSummariesAtsRepository.executeCustomQuery(
        sql,
        [id],
      );
