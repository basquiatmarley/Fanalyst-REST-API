...
export class UsersPredictionsController {
  ...
  async create(
    ...
    const saved =
      await this.usersPredictionsRepository.create(usersPredictions);
    ...
    if (!getOneSummary) {
      ...
      await this.usersPredSummaryRepo.updateById(getOneSummary.id, {
        countPrediction: getOneSummary.countPrediction + 1,
      });
    }
    //ALL TIMES
    ...
    if (!getOneSummaryAts) {
      ...
      await this.usersPredSummaryAtsRepo.updateById(getOneSummaryAts.id, {
        countPrediction: getOneSummaryAts.countPrediction + 1,
      });
    }
    return saved;
  }
  ...
  async findById(
    ...
    @param.filter(UsersPredictions, {exclude: 'where'})
    filter?: FilterExcludingWhere<UsersPredictions>,
  ): Promise<UsersPredictions> {
